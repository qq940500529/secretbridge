...
> [!IMPORTANT]
> 当前版本为 `0.1.0-alpha.20` 工程候选，处于 **M3 安全试点准备**。M1 本地工作流与 M2 受控操作已经完成；M0 中安装后操作系统身份隔离仍未关闭。低权限试点工作台只形成治理记录，不会验证外部授权或自动执行真实试点，也不等于已经适合生产使用。
...
**仍待外部执行**：由目标负责人提供专用非生产账号、书面授权、网络窗口和证书链；独立人员复核远端授权集；在获批环境逐项执行成功、拒绝、超时、DNS、TLS、证书、断网、轮换、撤销、重启和清理验证并关联外部证据。本阶段没有收到这些材料，因此未发起真实连接。

**验收**：准入无技术失败且外部门槛有引用；远端权限由独立人员确认；失败矩阵和恢复可重复；不触碰生产，不扩大固定 SQL 或结果范围。

### 0.1.0-alpha.21 · 安全复核与整改闭环（待开始，M3）
...
3. 仅在获得专用非生产 PostgreSQL 环境和负责人授权后启动 alpha.20；
4. 由非原实现人员执行 alpha.21 独立安全复核；
