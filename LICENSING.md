...
应用首次运行时展示的[最终用户许可与免责声明](./docs/最终用户许可与免责声明.md)用于确认用户已阅读 Beta 风险、安全边界和使用责任，不修改或增加 AGPL 的许可条件，也不是取得源代码许可的前提。
...
外部贡献默认依贡献时明确的开源条款处理。仅有DCO签署不等于授予公司闭源再许可权；未获充分授权的贡献不得自动纳入商业授权范围。详见[贡献与再许可政策](./docs/governance/贡献与再许可.md)和[依赖许可风险评估](./docs/governance/依赖许可风险评估.md)。
