...
回退切换安装记录中的上一版本，不是恢复业务数据。当前开发版本使用 schema 23；此预发布功能不承诺与旧版 PIN/TOTP 身份验证配置兼容。旧程序不能打开新数据库，因此回退程序前必须恢复与旧版本匹配的独立备份。回退命令不会撤销数据库变更。恢复流程见[配置迁移与备份恢复](../user-guide/备份与恢复.md)。
