...
| `client-config CLIENT` | 打印当前安装版本的客户端 MCP 配置；未列出的本机 stdio MCP 客户端使用 `generic`，命令不修改客户端文件 |
...
`status` 的 `installation.binary` 和 `install` 成功 JSON 中的 `binary` 都会给出当前已安装版本的绝对程序路径。使用该程序的 `client-config CLIENT` 生成已列出客户端的专用 MCP 配置；其他支持本机 stdio MCP 的客户端用 `client-config generic` 取得通用项，再按宿主格式映射。支持列表、合并位置与验证方法见[AI 客户端接入](../../CLIENT_INTEGRATIONS.md)。不要使用临时解压目录或源码构建目录中的程序。较早的发行包可能尚无此命令，应遵循**实际安装版本**的文档，不要把新格式套在旧版本上。

不同 AI 客户端的配置文件位置、字段外层和重载方式可能不同，应以该客户端当前说明为准。自动部署助手只有在能够可靠识别目标配置时才能修改，并须先展示配置路径、备份和最小差异；无法确定时应给出可粘贴片段，不得猜测路径或覆盖其他 MCP 服务。

生成结果显式包含当前数据目录的 `SECRETBRIDGE_DATA_DIR`，确保桥接与代理使用同一绝对路径。配置中不需要、也不得加入凭据、页面配对链接或内部桥接令牌。
...
每次升级或回退都会输出当前活动版本的 `binary`。重新生成客户端配置，核对并更新 MCP 程序路径，重载客户端并重复只读验证。卸载前先从 AI 客户端移除该 MCP 项；否则客户端会继续尝试启动已经不存在的程序。
