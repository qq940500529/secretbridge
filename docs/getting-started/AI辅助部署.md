...
7. 把 SecretBridge 接入我使用的 MCP AI 客户端。优先识别当前客户端，无法确定时只询问一次。若已安装程序支持 `client-config CLIENT`，已列出的客户端使用专用配置；其他支持本机 stdio MCP 的客户端使用 `client-config generic`，并按宿主格式映射字段（详见仓库根目录 `CLIENT_INTEGRATIONS.md`）。旧发行包没有该命令时，按其随包文档配置，不要凭新版格式猜测。先展示并备份将修改的配置，获得同意后，只合并 SecretBridge 项；本机数据目录必须与代理一致，配置不得包含凭据、页面配对令牌或其他秘密。重载 AI 客户端，确认 SecretBridge 工具可见，并调用只读的 secretbridge_terminal_capabilities 验证连接。若不能安全自动配置，提供可直接粘贴的配置和准确重载步骤，不要猜测配置路径。
...
2. 按[客户端接入指南](../../CLIENT_INTEGRATIONS.md)选择当前客户端，用支持该命令的已安装程序执行 `client-config CLIENT`；未列出的本机 stdio MCP 客户端执行 `client-config generic` 并按实际宿主格式映射。命令只打印包含当前活动程序路径和数据目录的配置，不会修改客户端文件或输出凭据。旧发行包没有该命令时，以其随包文档为准。
3. 只有在配置文件和目标客户端可以可靠确认时才自动修改；修改前展示路径、备份位置和最小差异，并等待用户同意。合并生成结果中的 SecretBridge 项，不覆盖其他服务器。生成配置明确传递当前数据目录，避免客户端环境与代理环境不一致。
...
MCP 配置不需要密码、页面配对链接或桥接令牌。桥接从相同数据目录中的本机私有连接信息接入代理；不要复制该内部文件。升级或回退后应重新运行 `client-config CLIENT`，并核对客户端中的程序路径是否与当前 `status` 的 `installation.binary` 一致。
