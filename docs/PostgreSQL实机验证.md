...
本文说明如何在隔离的临时 PostgreSQL 上复现 SecretBridge 固定只读适配器的真实网络、TLS 和认证验证。它补充自动化替换执行器测试，但不等同于生产目标试点、第三方安全审计或三平台认证。
...
```sh
cargo test -p secretbridge-server \
...
[使用指南](使用指南.md) · [安全模型与验收](安全模型与验收.md) · [低权限试点工作台](低权限试点工作台.md) · [完整路线图](../ROADMAP.md)
