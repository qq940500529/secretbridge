...
面向平台开发者、产品设计者与测试参与者。本文定义三平台适配合同、Web界面信息结构和体验验收目标；管理界面已具备端到端工作流，原生凭据库与进程功能仍待完整的平台验证。
...
以上为开源版 Beta 首轮验证目标，不是已证实的最低 OS 版本或完整兼容保证。Windows/Linux arm64、macOS Intel 与其他发行版标记实验性，直到获得独立设备证据。不能把 GitHub 托管 runner 测试当作原生凭据库与浏览器交互验收。最低版本与架构的证据、复核日期和失效规则见[支持矩阵](../release/支持矩阵与性能基线.md)。
...
当前Web界面已经提供键盘可见焦点、跳到主要内容、折叠导航的可访问名称、连接状态播报、减少动态偏好和窄视口导航。上述为实现基线，不替代Windows、Linux和macOS上的200%缩放、浏览器键盘路径及屏幕阅读器人工验收。
...
运行界面只呈现用户任务、当前状态、风险和下一步操作，不展示版本里程碑、开发阶段、路线图或“已实现某技术”的宣传性标签。版本与开发计划属于文档、发行说明和诊断信息，不占用日常业务界面的视觉层级。

页面遵循稳定的企业工作流结构：标题与页面级操作位于首部；说明性风险使用低强调通知条；新增和编辑按需展开或进入独立流程；主数据使用表格或连续行列表；需要审阅的对象使用主列表—详情工作区；状态是记录的字段而不是独立卡片。除终端、报告详情、空状态和确有独立边界的工作区外，不用卡片表达普通字段组，也不把新增、列表和状态三个容器并排堆叠。

同一页面只保留一个主要动作。批准、拒绝、撤销、取消等上下文动作跟随选中记录或详情区出现；危险动作与主要动作在颜色、位置和确认语义上分离。列表行优先保证名称、状态、对象、更新时间和必要版本可扫描，其余信息按需展开。
