...
本页帮助贡献者获取仓库、运行现有检查并提交可审阅的变更。应用架构与计划接口位于[开发设计](./架构设计.md)。
...
无参数命令启动脱离终端的后台；前台调试用 `cargo run -p secretbridge -- --serve`。后台需要停止时运行 `cargo run -p secretbridge -- stop`，不是关闭启动终端。安装包构建和三平台隔离验收见[后台运行与安装交付](../getting-started/安装与服务管理.md)。

```sh
cargo run -p secretbridge
```

服务只绑定回环地址并打开系统默认浏览器。前端开发时可分别运行`cargo run -p secretbridge`和`pnpm dev:web`；Vite仅监听`127.0.0.1:5173`并代理`/api`。本项目采用浏览器管理端，不使用桌面壳。
...
```sh
cargo run -p secretbridge -- --mcp-stdio
```

桥接进程的stdout专用于MCP JSON-RPC，调试日志必须保留在stderr。它读取与后台代理相同应用数据目录中的`mcp-bridge.json`，并通过Windows命名管道或Unix domain socket连接代理；它不打开浏览器、不加载SQLite、不持有Web监听端口，连接结束也不会停止后台代理。协议、令牌轮换和生命周期测试位于`backend/src/adapters/mcp/mod.rs`，使用合成状态与临时本机IPC端点，不连接真实业务目标。

数据目录、桥接连接文档和本机 IPC 端点权限由 `backend/local-access` 辅助包按平台实现：Windows 使用原生 DACL，Linux/macOS 使用私有文件与 socket 权限。代理本体继续禁止 unsafe 代码；跨账号访问验证见[社区验证待办](../community/社区验证待办.md)。

Unix 桥接在发布连接文档前核对 socket 与私有数据目录的所有者一致；连接端读取文档时再核对父目录、文档和 socket 的所有者与私有权限。自动化测试覆盖权限过宽、目录符号链接和合法的残留 socket 恢复；另一个系统账号的真实连接尝试仍列在社区验证待办。
...
├── backend/                      本机 Rust broker、CLI 与跨平台权限辅助包
├── frontend/                     React／TypeScript 管理端
├── brand/                        Logo 原图与高保真 SVG
├── docs/                         使用、实现与维护文档
├── CLIENT_INTEGRATIONS.md        专用及通用 AI 客户端配置
├── skills/secretbridge-operations/  可安装的 AI 操作 Skill 与合成示例
├── tools/checks/check_repository.py    有限仓库检查
├── tests/                       检查器测试
├── .github/                     CI、评审与问题模板
...
真实协议、认证、操作、取消和数据重开的验收在 Rust 测试中执行。数据库可通过 `bash tools/acceptance/test_database_connectors.sh` 建立仅回环暴露的临时 Docker 服务、合成凭据和测试证书；要求 Docker、OpenSSL 与 Rust。脚本仅清理自身唯一命名的容器和证书，详见[数据库凭据任务](../user-guide/tasks/数据库凭据任务.md)。另有交互式浏览器脚本检查 SFTP/Git/数据库编辑器的输入、执行方式切换及实际提交配置；该脚本模拟 API，不触碰系统凭据库或真实业务服务。

启动 Web 开发服务器：

```sh
pnpm --dir frontend exec vite --host 127.0.0.1 --port 8799 --strictPort
```

安装仓库锁定的 Playwright 浏览器后，可执行单项脚本或汇总验收：

```sh
node tools/acceptance/smoke_connector_ui.mjs
...
保持术语一致：credential为“凭据”，broker为“执行代理”，approval为“审批”，session为“会话”。首次使用缩写时链接[术语表](../user-guide/术语表.md)。
...
继续阅读：[开发设计](./架构设计.md) · [安全模型与验收](../security/安全模型.md) · [贡献指南](../../CONTRIBUTING.md)
