...
本文说明 `0.2.0-beta.4` 的当前模块和不可跨越的安全边界。未来工作只在[路线图](../ROADMAP.md)和 GitHub Issues 跟踪，不在这里维护一套假想架构。
...
`secretbridge-core::SCHEMA_VERSION` 是 schema 的唯一事实源，当前值为 16。首个公开发行前不兼容旧开发数据库或旧安装包：打开非空且非 16 的数据库会直接拒绝，并且不修改原文件。恢复也只写入新的绝对目录。
