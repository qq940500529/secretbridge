...
本文说明 `0.2.0-beta.5` 的当前模块和不可跨越的安全边界。未来工作只在[路线图](../ROADMAP.md)和 GitHub Issues 跟踪，不在这里维护一套假想架构。
...
| `catalog/schema.rs` | 新数据库的 schema 16 创建逻辑 |
...
`secretbridge-core::SCHEMA_VERSION` 是 schema 的唯一事实源，当前值为 16。首个公开 Beta 不兼容此前未发布的旧开发数据库或旧安装包：打开非空且非 16 的数据库会直接拒绝，并且不修改原文件。恢复也只写入新的绝对目录。后续公开 Beta 的兼容范围以对应 Release 说明为准。
...
- 启动令牌位于 URL fragment，只能配对一次；页面会话可撤销、会过期，并在服务重启后失效。
...
普通终端启动本机 Shell，支持断线重连、64 KiB 有界回放、字节游标、尺寸调整和单写入者租约。它与凭据任务是两条独立路径：不会注入系统凭据，也不承诺过滤任意终端内容。代理退出后旧 PTY 不会伪装成已恢复。
