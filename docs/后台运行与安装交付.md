...
回退切换安装记录中的上一版本，不是恢复业务数据。首个公开 Beta `0.2.0-beta.5` 仅接受 schema 16，不迁移此前未发布的开发数据库，也不能用此命令撤销数据库变更或任意降级。需要保留旧测试数据时先复制原目录，再使用新数据目录验证当前版本；后续公开 Beta 的兼容范围以对应 Release 说明为准。恢复流程见[配置迁移与备份恢复](配置迁移与备份恢复.md)。
