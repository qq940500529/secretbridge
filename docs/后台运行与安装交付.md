...
回退切换安装记录中的上一版本，不是恢复业务数据。当前未发布开发版使用 schema 18，不兼容更早的开发数据库；公开 Beta 之间能否回退以对应 Release 说明为准。回退命令不会撤销数据库变更。需要保留旧测试数据时先复制原目录，再使用新数据目录验证当前版本。恢复流程见[配置迁移与备份恢复](配置迁移与备份恢复.md)。
