...
回退切换安装记录中的上一版本，不是恢复业务数据。当前开发版本使用 schema 22，可从公开 Beta 7 的 schema 18 及 Beta 8 的 schema 19 原位升级；旧程序不能打开升级后的数据库，因此回退程序前必须恢复与旧版本匹配的独立备份。更早的开发数据库不受支持。回退命令不会撤销数据库变更。恢复流程见[配置迁移与备份恢复](配置迁移与备份恢复.md)。
