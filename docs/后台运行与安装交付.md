...
回退切换安装记录中的上一版本，不是恢复业务数据。当前未发布 Beta 仅接受 schema 16 的包；首个公开版本前不承诺兼容旧 Beta 数据，也不能用此命令撤销数据库变更或任意降级。需要保留旧测试数据时先复制原目录，再使用新数据目录验证当前版本；恢复流程见[配置迁移与备份恢复](配置迁移与备份恢复.md)。
