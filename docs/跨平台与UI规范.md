...
设计v0.2，2026-09-10。三平台均为正式目标，不再将Linux/macOS推迟为可选扩展；当前均未实现或完成运行验证。
...
| UI引擎 | WebView2 | WebKitGTK | WKWebView |
...
| 发布目标 | 签名MSI或NSIS安装包 | 明确发行版的deb与经测试的AppImage | 签名、公证、staple验证的dmg/app |

以上为项目测试范围提案，不是各组件最低版本或完整兼容保证。Windows/Linux arm64与其他发行版标记实验性直到有独立证据；macOS两架构分别构建或验证Universal包。不能把GitHub托管runner文档测试当作原生凭据库与UI验收。
...
桌面布局：左侧导航＋顶部连接／隔离状态＋主工作区；审批详情可侧栏展开。终端标签展示环境、连接状态和输入权。小窗口折叠导航，不隐藏关键审批信息。初次打开显示“无真实凭据、先用模拟任务”，引导配置目标→操作→批准→结果，每步可撤回。
...
- 交互响应目标：本地点击可见反馈p95≤100ms，窗口初次可交互≤3秒；需定义硬件、样本和冷启动条件后实测，不作为当前性能事实。
- 数据量测试：至少100目标／1000审计项仍可筛选，终端大输出不阻塞审批；使用虚拟列表与背压，性能优化不能绕过输出审查。

UI原型、截图对比、键盘测试、屏幕阅读器检查、原生三平台试用应分别留证。尚未生成产品截图，不使用伪造UI截图作为完成证明。

## 官方依据

- [Tauri WebView差异](https://v2.tauri.app/reference/webview-versions/)：三平台渲染引擎不同，必须跨引擎验证。
