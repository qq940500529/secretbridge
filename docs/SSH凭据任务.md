...
SSH exec 协议传输的是一条字符串而不是 argv。代理对程序路径和每个参数分别进行 POSIX 单引号引用，保留空字符串、空格、换行、Unicode、单引号及字面量 `$(...)`，不递归展开插槽文本。认证秘密不会加入这条命令，不写入临时私钥文件或普通终端环境。
...
Git 与 SFTP 文件传输是独立的后续交付，不属于当前 SSH 命令能力。

---

[使用指南](使用指南.md) · [功能路线图](../ROADMAP.md) · [开发设计](开发设计.md)
