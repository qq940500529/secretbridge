...
Windows 数据目录、桥接连接文档和命名管道的安全描述符由独立的 `secretbridge-windows-pipe-acl` crate 通过 Windows API 创建与检查；代理本体继续禁止 unsafe 代码。Windows CI 会检查这三处的原生 DACL，跨账号访问仍列在[社区验证待办](社区验证待办.md)。
