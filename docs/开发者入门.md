...
| Rust／Node.js | 当前检查不需要 | 应用实现后按锁定工具链说明安装 |

现有检查只使用Python标准库，无需安装第三方Python包、填写环境变量或准备业务系统连接。
...
预期结果：仓库检查显示通过，单元测试以 `OK` 结束，差异检查没有格式错误。普通贡献者可以先fork，再对自己的fork执行以上步骤，向上游main发起PR。

> [!NOTE]
> 这些命令不会启动密桥，也不验证操作系统凭据库。当前仓库没有应用运行入口或安装包构建命令。
