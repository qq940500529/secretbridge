...
调试MCP stdio模式前先执行`pnpm build`，再由支持命令式stdio服务的MCP宿主启动：

```sh
cargo run -p secretbridge-server -- --mcp-stdio
```

此时stdout专用于MCP JSON-RPC，调试日志必须保留在stderr。该命令会同时启动Web控制台并占用默认回环端口；不要与普通模式并行启动。协议测试位于`crates/secretbridge-server/src/mcp.rs`，使用内存双工流，不连接真实网络目标。
