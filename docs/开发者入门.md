...
真实协议、认证、操作、取消和数据重开的验收在 Rust 测试中执行。另有交互式浏览器脚本检查 SFTP/Git 编辑器的输入、执行方式切换及实际提交配置；该脚本模拟 API，不触碰系统凭据库或真实业务服务。
...
在已安装 Playwright 及其 Chromium 的开发环境中，另开终端执行：

```sh
node tools/smoke_connector_ui.mjs
```

可通过 `PLAYWRIGHT_MODULE` 指定外部 Playwright 模块位置；`PLAYWRIGHT_CHANNEL` 可选已安装的 `msedge` 或 `chrome`，`SECRETBRIDGE_UI_URL` 可指定另一个本机预览地址。Playwright 不是应用运行依赖，也不进入默认 CI 必需依赖清单。测试浏览器始终以无窗口方式运行，退出后关闭；测试完成后停止开发服务器。
