...
PR CI 在 Linux、Windows 和 macOS 上保留上述检查及依赖许可证、安装包和个人规模验收，并在 Windows/macOS 上额外运行原生凭据库测试。完成一次原生构建后，安装包及隔离生命周期验收会与 Rust 测试、Clippy 并行运行；固定版本的漏洞扫描器按策略和工具链缓存。首次无缓存运行仍需编译扫描器，任何并行分支失败都会使对应 CI 任务失败。
