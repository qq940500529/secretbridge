...
桥接进程的stdout专用于MCP JSON-RPC，调试日志必须保留在stderr。它读取与后台代理相同应用数据目录中的`mcp-bridge.json`，不打开浏览器、不加载SQLite、不持有Web监听端口；连接结束也不会停止后台代理。协议和生命周期测试位于`crates/secretbridge-server/src/mcp.rs`，使用合成状态和回环临时端口，不连接真实业务目标。
