...
通过原生 PostgreSQL 或 MySQL 驱动执行登记的操作。密码只用于驱动认证，不进入 SQL、命令行、连接 URL、临时认证文件或普通终端；智能助手使用任务名称、普通参数和已批准的输出。
...
支持 PostgreSQL 14+、MySQL 8.0+。MySQL 查询使用 `max_execution_time`；当前不宣称支持 MariaDB。连接器不捆绑数据库、客户端命令行程序或容器镜像。Docker 仅用于可选的开发验收。
...
脚本建立唯一命名、仅回环映射的临时 PostgreSQL/MySQL 容器、合成密码与两天有效期的测试证书，结束时删除自身容器和证书目录。不操作业务实例。CI 独立运行此验收；常规三平台回归默认跳过需要这些真实服务的测试。
