...
**评估日期：2026-09-12。适用对象：当前锁定依赖与后续候选组件，不是最终发行物合规证明。**
...
| reqwest 0.13.5，MCP私有回环桥接客户端 | [官方crate清单](https://docs.rs/crate/reqwest/0.13.5/source/Cargo.toml)声明MIT或Apache-2.0；关闭默认feature，仅启用`json` | 运行时代码进入服务包，发行时保留所选许可及适用通知；未启用TLS、HTTP/2、系统代理、压缩、Cookie或multipart，客户端代码仍显式禁用代理与重定向 | 较低／已锁定；仅用于字面回环HTTP，不作为跨主机客户端 |
| base64 0.23.1、tower-http 0.6.11等reqwest新增间接依赖 | crate元数据分别声明MIT或Apache-2.0、MIT；确切图由`Cargo.lock`解析 | 许可表达式已进入门禁允许表；与仓库已有不同版本并存时，发行清单必须按版本分别列出，不能合并成一个组件 | 较低／已记录 |
