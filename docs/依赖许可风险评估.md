...
| rustls 0.23.44、rustls-platform-verifier 0.7.0、ring 0.17.14等新增TLS间接依赖 | 锁文件元数据为Apache-2.0／ISC／MIT组合，具体表达式由许可门禁解析；`webpki-root-certs`的根证书数据声明CDLA-Permissive-2.0 | 平台验证器会调用各操作系统信任服务；发行物必须按平台生成实际SBOM和通知，不把系统证书服务视为仓库可再许可内容。若分发根证书数据，保留CDLA许可与来源记录 | 较低到条件性／已记录，需按实际平台产物复核 |
