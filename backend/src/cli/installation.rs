...
fn manifest(root: &Path, check_files: bool) -> Result<Manifest> {
    ...
    if manifest.format != "secretbridge-package"
        ...
        || manifest.schema_version != secretbridge::SCHEMA_VERSION
...
async fn activate(
    ...
    let activation = async {
        let status = launch(&release.join(binary_name()), &release.join("frontend")).await?;
    ...
    if activation.is_err() {
        ...
        if let Some(active) = &old.active {
            ...
            launch(&prior.join(binary_name()), &prior.join("frontend"))
                .await
...
mod tests {
    ...
    fn package_paths_reject_traversal_absolute_and_platform_escapes() {
        ...
        assert_eq!(
            safe_path(&root, "frontend/assets/app.js").unwrap(),
            root.join("frontend/assets/app.js")
        );
    ...
    fn sbom_must_match_package_version_and_target() {
        ...
        let manifest = Manifest {
            ...
            schema_version: secretbridge::SCHEMA_VERSION,
            files: Vec::new(),
        };
