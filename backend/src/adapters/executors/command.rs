...
use crate::parameters::identifier;
use crate::{
    AppState,
...
pub async fn drive(state: &AppState, id: Uuid, cancellation: &CancellationToken) {
    let _permit = tokio::select! {
        ...
        () = cancellation.cancelled() => { crate::invalidate_synthetic_run(state.catalog.clone(),id).await; return; }
    };
    ...
    let Ok(Ok(context)) = context else {
        crate::invalidate_synthetic_run(state.catalog.clone(), id).await;
        return;
    };
    ...
    let remaining = context
        .approval
        .expires_at_unix_ms
        .saturating_sub(crate::now_unix_ms());
    let limit =
