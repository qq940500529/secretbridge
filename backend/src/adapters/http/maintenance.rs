...
use super::{
    ApiError, AppState, ConfigurationStorage, DefaultBodyLimit, IntoResponse, Json, Method, Next,
    Request, Response, Router, Serialize, State, catalog, get, map_catalog_error, middleware, post,
    require_session, task, validate_origin,
};
use crate::{BackupReport, SecretState};
use axum::body::Bytes;
...
async fn diagnostics(State(state): State<AppState>) -> Result<Json<Diagnostics>, ApiError> {
    task::spawn_blocking(move || {
        ...
        Ok(Json(Diagnostics {
            ...
            bridge_schema_version: crate::adapters::mcp::BRIDGE_CONNECTION_SCHEMA,
            authentication_mode,
