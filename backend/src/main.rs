...
use secretbridge::{AppState, LocalMcpBridge, router_with_web, serve_mcp_stdio_bridge};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        return cli::synthetic_child::run_synthetic_terminal();
    }
    if run_maintenance(&arguments)? {
        return Ok(());
    }
    if cli::handle(&arguments).await? {
        return Ok(());
    }
    let startup_mode = parse_startup_mode(&arguments)?;

    let log_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("secretbridge=info"))
    ...
    cli::validate_web_build(&web_root)?;
...
fn run_maintenance(arguments: &[std::ffi::OsString]) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    let report = if arguments[0] == "--inspect-backup" {
        secretbridge::inspect_configuration_backup(&path)?
    } else {
        let directory = env::var_os("SECRETBRIDGE_DATA_DIR")
            .map(PathBuf::from)
            .ok_or("restore_requires_explicit_data_directory")?;
        secretbridge::restore_configuration_backup(&path, &directory)?
    };
...
fn create_private_data_directory(path: &Path) -> io::Result<()> {
    ...
    secretbridge_local_access::protect_directory(path)
}
...
fn default_web_root() -> PathBuf {
    ...
    {
        let packaged = root.join("frontend");
...
fn application_log_target(target: &str) -> bool {
    target == "secretbridge" || target.starts_with("secretbridge::")
}
...
mod tests {
    ...
    fn global_trace_logging_cannot_enable_raw_protocol_output() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info!(target:"secretbridge::command", "fixed-application-status");
            ...
            tracing::warn!(target:"secretbridge_other", "synthetic-unfiltered-protocol-value");
        });
    ...
    use super::{
        StartupMode, bind_local_bridge, parse_startup_mode, require_loopback,
        resolve_data_directory,
    };

    #[test]
    fn bridge_startup_failure_is_fixed_and_recorded_without_the_path() {
        let (state, _) = secretbridge::AppState::new([]);
        let path =
    ...
    fn synthetic_stress_arguments_are_bounded() {
        use crate::cli::synthetic_child::bounded_argument;
        assert_eq!(
