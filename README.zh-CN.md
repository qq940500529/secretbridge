...
> [!IMPORTANT]
> 当前版本是功能开发中的本机原型。本机Web管理端可以把密码和API令牌写入操作系统凭据库；本地MCP stdio服务可以申请审批、启动已批准的单次运行、查询状态／事件以及取消活动运行。秘密值没有读取或导出工具／接口。真实系统终端、通用凭据注入和更多连接器是接下来的开发重点。
...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的合成PTY。凭据元数据、连接目标、模板、审批、运行与审计在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库。PostgreSQL适配器只执行固定只读检查并返回枚举状态，支持显式配置私有 CA 文件；每条批准最多消费一次，配置漂移默认失效。MCP只暴露固定工具，不包含审批决定、取密、SQL、已注入凭据的命令或连接串。schema v12 已移除安全证据、试点准入和试点治理等超前产品层，开发重新聚焦真实终端与凭据代执行。详见[功能优先路线图](ROADMAP.md)。
...
服务仅监听`127.0.0.1:8787`并打开系统默认浏览器。启动令牌位于URL片段中，页面读取后立即清除；换页或刷新不会持久化会话。“凭据”页把密码和API令牌直接写入操作系统凭据库，不提供回读；“连接目标”页保存经过校验的PostgreSQL地址元数据，不保存连接串或密码，并固定使用`verify_full` TLS。“执行策略”“审批中心”“运行任务”和“审计记录”可驱动固定PostgreSQL状态检查或离线合成检查，两者都不接受调用方SQL。“安全终端”目前仍运行内置合成程序，下一版本将接入PowerShell、CMD、Bash和Zsh真实终端。该项目使用浏览器Web界面，不计划引入桌面壳。
...
项目处于功能开发阶段，当前优先完成真实终端、AI即时控制、凭据注入、任务模板和常用连接器。欢迎参与跨平台实现、UI、测试与文档完善。开始贡献前请阅读[贡献指南](CONTRIBUTING.md)与[开发者入门](docs/开发者入门.md)。
