...
> [!IMPORTANT]
> **当前进入M3安全试点准备，M0身份边界门槛仍未关闭。** 本机Web管理端可以把密码和API令牌写入操作系统凭据库；本地MCP stdio服务可以申请审批、启动已批准的单次运行、查询受限状态／事件以及取消活动运行。“安全验证”页可运行当前实例检查和隔离攻防场景，持久化固定证据并导出可校验摘要的报告。审批决定仍只能在可信Web界面完成。秘密值没有读取或导出工具／接口，任意SQL和带凭据Shell仍未开放。
