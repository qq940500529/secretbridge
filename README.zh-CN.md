...
当前已实现合成模式状态接口、一次性配对、可过期／撤销的页面会话，以及可断线重连的合成PTY；其余能力及准入条件见[路线图](ROADMAP.md)。
...
服务仅监听`127.0.0.1:8787`并打开系统默认浏览器。启动令牌位于URL片段中，页面读取后立即清除；换页或刷新不会持久化会话。“安全终端”页面只能运行密桥内置的回显／状态程序，不会启动PowerShell、`cmd`、`sh`或用户命令；WebSocket断开和重连不会终止该合成PTY。该原型不使用、也不计划引入桌面壳。
