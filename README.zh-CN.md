...
> [!IMPORTANT]
> 当前版本是功能开发中的本机原型。本机Web管理端可建立可重连的PowerShell／CMD、Bash或Zsh终端，并把密码和API令牌写入操作系统凭据库；本地MCP stdio服务可以申请审批、启动已批准的单次运行、查询状态／事件以及取消活动运行。秘密值没有读取或导出工具／接口。AI终端控制、通用凭据注入和更多连接器是接下来的开发重点。
...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的真实PTY。后台按平台探测可用Shell，浏览器断开后进程继续运行，近期输出可按单调游标恢复；创建时可以指定会话名称、存在的绝对工作目录和有界的普通环境变量。凭据元数据、连接目标、模板、审批、运行与审计在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库。PostgreSQL适配器只执行固定只读检查并返回枚举状态，支持显式配置私有CA文件；每条批准最多消费一次，配置漂移默认失效。MCP只暴露固定工具，尚不包含终端控制，也不包含审批决定、取密、SQL、已注入凭据的命令或连接串。详见[功能优先路线图](ROADMAP.md)。
