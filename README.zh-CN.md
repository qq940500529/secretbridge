...
> [!IMPORTANT]
> SecretBridge 是面向个人、单机使用的预发行软件，已经具备真实代理、Web、终端、系统凭据库、连接器和安装生命周期实现，但尚未发布公开 GitHub Release。评估源码构建时只使用合成凭据。秘密值没有读取或导出工具／接口；普通终端不注入凭据，输出过滤也不等于程序沙箱。
...
当前公开仓库交付面向个人用户、单机运行的完整开源版。未来闭源商业企业版将围绕组织身份、集中策略、受管节点、多人审批和集中审计另行开发，不作为开源版 1.0 的前置条件。详见[开源版与企业版产品边界](docs/开源版与企业版.md)。

当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的真实PTY。后台按平台探测可用Shell，浏览器断开后进程继续运行，近期输出可按单调游标恢复；创建时可以指定会话名称、存在的绝对工作目录和有界的普通环境变量。Web与MCP共享终端，由独立输入租约协调操作；审批、任务和终端列表支持实时刷新。凭据元数据、连接目标、模板、审批、运行与审计在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库。PostgreSQL适配器只执行固定只读检查并返回枚举状态，支持显式配置私有CA文件；单次批准最多消费一次，限时授权只重复已确认参数，配置漂移默认失效。MCP没有审批决定或取密工具；普通终端不会注入凭据，也不宣称能脱敏任意文件或命令输出。详见[AI终端与实时状态](docs/AI终端与实时状态.md)及[开源版路线图](ROADMAP.md)。

## 一次操作如何完成？

Web 工作台提供“凭据、连接、任务、终端、历史、设置”六个入口，使用可搜索的主从列表和右侧滑入编辑抽屉；连接详情可直接进入任务授权或最近执行。参见[工作台使用说明](docs/工作台使用说明.md)。

原生 [HTTP 凭据任务](docs/HTTP凭据任务.md)支持固定接口、请求头与 JSON 请求体认证、普通参数和返回字段筛选；无需把令牌交给助手，也无需安装 curl。HTTPS 使用平台信任根，默认只返回状态码。

[SSH 凭据任务](docs/SSH凭据任务.md)支持可信主机指纹、密码或 Ed25519/ECDSA 私钥认证、固定 POSIX 远程命令与脱敏流式结果。[文件传输与 Git 任务](docs/文件传输与Git任务.md)支持 SFTP 单文件上传／下载、显式覆盖和 Git HTTPS 令牌认证的远端分支查询、拉取与非强制推送。[数据库凭据任务](docs/数据库凭据任务.md)提供 PostgreSQL/MySQL 连接检查、版本查询和登记的只读参数化查询，支持指定返回列、保留数值精度与表格结果。当前不包含 Git SSH 或自由 SQL 控制台。
...
| 了解开源版与未来企业版的差异 | [开源版与企业版产品边界](docs/开源版与企业版.md) |
...
首次部署优先建议使用能够在本机展示命令并等待确认的可信 AI 编程助手。请直接使用 [AI 辅助部署](docs/AI辅助部署.md)中的说明和可复制提示词；同页也提供供 AI 读取的执行引导，以及完全不依赖 AI 的人工部署步骤。不要向助手提供真实秘密，也不要绕过失败的质量门禁。
...
启动命令返回后服务仍在后台运行。可使用 `start --no-open`、`open`、`status` 和 `stop` 控制；前台调试使用 `--serve`。版本化安装、升级/回退、登录自启动、卸载数据选择见[后台运行与安装交付](docs/后台运行与安装交付.md)。原生包包含配套 Web、对应源码、第三方许可和 CycloneDX SBOM；安装前可用 `verify-package` 离线校验，详见[发行物验证与 SBOM](docs/发行物验证与SBOM.md)。当前仍是未签名开发包。
...
“设置 → 数据维护”提供非秘密配置导入/导出、SQLite 一致性备份、只读恢复预检和精简诊断。配置导入采用追加策略；维护命令只恢复到全新目录，原目录保留用于回退。迁移与恢复都需要重新录入秘密值并重新授权；文件隐私边界和跨平台路径处理见[配置迁移与备份恢复](docs/配置迁移与备份恢复.md)。
