...
[SSH 凭据任务](docs/SSH凭据任务.md)支持可信主机指纹、密码或 Ed25519/ECDSA 私钥认证、固定 POSIX 远程命令与脱敏流式结果。[文件传输与 Git 任务](docs/文件传输与Git任务.md)支持 SFTP 单文件上传／下载、显式覆盖和 Git HTTPS 令牌认证的远端分支查询、拉取与非强制推送。Git SSH、MySQL 及数据库查询仍在后续计划中。
