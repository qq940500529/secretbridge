...
> [!IMPORTANT]
> 当前版本是功能开发中的本机原型。本机Web管理端可建立可重连的PowerShell／CMD、Bash或Zsh终端，并把密码和API令牌写入操作系统凭据库；本地MCP stdio服务可操作普通终端、申请审批、启动已批准的单次运行、查询状态／事件以及取消活动运行。秘密值没有读取或导出工具／接口。通用凭据注入和更多连接器是接下来的开发重点。
