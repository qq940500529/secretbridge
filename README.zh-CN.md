...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的合成PTY。凭据元数据与PostgreSQL目标信息在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库。PostgreSQL适配器只执行固定只读检查并返回枚举状态；每条批准最多消费一次，配置漂移默认失效。MCP只暴露8个固定工具，不包含审批决定、取密、SQL、Shell输入、连接串或目标地址。SQLite schema v11 保存可校验的安全验证、平台边界、试点准入、限时试点记录和逐项证据；报告不输出系统身份、路径、目标地址或秘密，也不声称完成授权或独立认证。状态接口仍明确标记“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[完整路线图](ROADMAP.md)。
