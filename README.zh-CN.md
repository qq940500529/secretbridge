...
当前已实现合成模式状态接口、一次性配对、可过期／撤销的页面会话、可断线重连的合成PTY，以及经过认证、可编辑并在本机SQLite中持久化的M1配置页面。“执行策略”页将内置合成操作和返回范围绑定到逻辑目标；审批中心基于已启用模板创建限时授权。每条已批准审批最多创建一个合成运行，请求可幂等重试，并可查看排队、运行、完成、取消、授权失效停止或重启中断状态；开始和完成前都会重新校验授权。“审计记录”只展示服务端生成的固定安全事件。整个运行过程只是内部短时状态模拟，不启动系统进程、不访问网络、不读取凭据、不接收操作参数。终端按输出游标续传未读内容，在多个连接间强制单写入者、其余只读；状态接口明确标记当前只是“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[路线图](ROADMAP.md)。
