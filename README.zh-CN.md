...
> [!IMPORTANT]
> 当前版本是功能开发中的本机原型。提供真实持久终端、操作系统凭据库、固定程序任务与限时审批；命名插槽支持标准输入、环境变量、参数和临时文件注入，Web与MCP可持续读取过滤后的运行输出。秘密值没有读取或导出工具／接口。普通终端仍不注入凭据，输出过滤不等于程序沙箱；更多连接器与参数化模板将继续完善。
