...
> [!IMPORTANT]
> **当前进入M1工作流开发，M0安全门槛仍未全部关闭。** 本机Web管理端现在可以把密码和API令牌写入操作系统凭据库，并把经过校验的PostgreSQL目标信息保存到本机SQLite；秘密值没有读取或导出接口。真实业务执行和系统Shell仍未开放，后续固定适配器必须接入现有策略与审批链。
...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的合成PTY。凭据元数据与PostgreSQL目标信息在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库，修改时要求已配对会话、精确Origin和预期版本，响应只显示“已配置／未配置”。“执行策略”、审批、运行和审计仍使用固定合成操作：每条批准最多消费一次，配置漂移默认失效，不启动网络连接。普通终端不会获得凭据。状态接口明确标记当前只是“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[路线图](ROADMAP.md)。
...
服务仅监听`127.0.0.1:8787`并打开系统默认浏览器。启动令牌位于URL片段中，页面读取后立即清除；换页或刷新不会持久化会话。“凭据”页把密码和API令牌直接写入操作系统凭据库，不提供回读；“连接目标”页保存经过严格校验的PostgreSQL地址元数据，不保存连接串或密码，并固定使用`verify_full` TLS。“执行策略”“审批中心”“运行任务”和“审计记录”仍只运行合成生命周期，不连接网络。“安全终端”只能运行密桥内置程序，不会启动PowerShell、`cmd`、`sh`或用户命令，也无法取得凭据。该原型不使用、也不计划引入桌面壳。
