...
> [!IMPORTANT]
> **当前进入M3安全试点准备，M0身份边界门槛仍未关闭。** 本机Web管理端可以保存系统凭据并完成受控审批/运行；MCP只能提出请求、查询、启动已批准的单次运行及取消。“安全验证”提供可重复自检证据，“试点准入”进一步汇总配置、自检与外部门槛，不会把技术就绪冒充业务授权。秘密值没有读取或导出工具／接口，任意SQL和带凭据Shell仍未开放。
...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的合成PTY。凭据元数据与PostgreSQL目标信息在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库。PostgreSQL适配器只执行固定只读检查并返回枚举状态；每条批准最多消费一次，配置漂移默认失效。MCP只暴露8个固定工具，不包含审批决定、取密、SQL、Shell输入、连接串或目标地址。SQLite schema v9 保存可校验的安全验证与试点准入证据；准入报告只输出计数和固定门槛，不输出目标地址，也不声称完成授权或独立认证。状态接口仍明确标记“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[完整路线图](ROADMAP.md)。
