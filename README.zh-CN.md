...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的合成PTY。凭据元数据与PostgreSQL目标信息在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库，修改时要求已配对会话、精确Origin和预期版本，响应只显示“已配置／未配置”。PostgreSQL适配器强制验证证书与主机名，在可串行化只读事务中执行固定`SELECT 1`，只返回枚举状态；每条批准最多消费一次，配置漂移默认失效。MCP只暴露8个固定工具，不包含审批决定、取密、SQL、Shell输入、连接串或目标地址。安全验证中心将服务器生成的证据保存在SQLite schema v8，并用SHA-256校验报告完整性；它不会连接真实目标，也不声称完成独立认证。普通终端不会获得凭据。状态接口明确标记当前只是“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[路线图](ROADMAP.md)。
