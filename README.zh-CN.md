...
> [!IMPORTANT]
> **当前处于M0安全基础阶段。** 仓库已有可运行的本机Web管理端、回环地址Rust服务、一次性浏览器配对与隔离的合成终端；真实凭据保存、注入、业务系统访问和系统Shell执行均被禁用。请勿配置真实凭据或连接业务系统。
...
当前已实现合成模式状态接口、一次性配对、可过期／撤销的页面会话，以及可断线重连的合成PTY。终端按输出游标续传未读内容，在多个连接间强制单写入者、其余只读，并限制向停滞浏览器写入的等待时间；状态接口明确标记当前只是“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[路线图](ROADMAP.md)。
...
服务仅监听`127.0.0.1:8787`并打开系统默认浏览器。启动令牌位于URL片段中，页面读取后立即清除；换页或刷新不会持久化会话。“安全终端”页面只能运行密桥内置的合成程序，不会启动PowerShell、`cmd`、`sh`或用户命令；WebSocket断开和重连不会终止该合成PTY，页面从上次输出游标继续读取。受限的`flood`和`wait`命令只用于韧性验证。该原型不使用、也不计划引入桌面壳。
