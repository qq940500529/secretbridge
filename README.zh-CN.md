...
启动命令返回后服务仍在后台运行。可使用 `start --no-open`、`open`、`status` 和 `stop` 控制；前台调试使用 `--serve`。版本化安装、升级/回退、登录自启动、卸载数据选择见[后台运行与安装交付](docs/后台运行与安装交付.md)。原生包包含配套 Web、对应源码与第三方许可，当前是未签名开发包。
