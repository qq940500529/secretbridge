...
> [!IMPORTANT]
> **当前进入M1工作流开发，M0安全门槛仍未全部关闭。** 仓库已有可运行的本机Web管理端、回环地址Rust服务、一次性浏览器配对、隔离的合成终端，以及使用本机SQLite持久化的凭据引用、逻辑目标、受控操作模板和范围化审批记录。配置不接收秘密值和网络地址；模板与审批记录也不会触发执行。真实凭据保存、注入、业务系统访问和系统Shell执行仍被禁用。
...
当前已实现合成模式状态接口、一次性配对、可过期／撤销的页面会话、可断线重连的合成PTY，以及经过认证、可编辑并在本机SQLite中持久化的凭据引用和逻辑目标配置页面。“执行策略”页可建立受控操作模板，将内置合成操作和返回范围绑定到一个目标；审批中心基于已启用模板创建1至60分钟的授权记录，并支持批准、拒绝、撤销、自动过期与并发版本保护。审批会保存模板版本及范围快照，但本阶段只记录授权意图，不执行操作。终端按输出游标续传未读内容，在多个连接间强制单写入者、其余只读，并限制向停滞浏览器写入的等待时间；状态接口明确标记当前只是“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[路线图](ROADMAP.md)。
...
服务仅监听`127.0.0.1:8787`并打开系统默认浏览器。启动令牌位于URL片段中，页面读取后立即清除；换页或刷新不会持久化会话。“凭据”“连接目标”“执行策略”和“审批中心”页面只接收非秘密、非地址配置，并保存到操作系统的本机应用数据目录。操作模板与审批中心当前没有执行按钮，也没有对应的命令或网络执行接口。“安全终端”页面只能运行密桥内置的合成程序，不会启动PowerShell、`cmd`、`sh`或用户命令；WebSocket断开和重连不会终止该合成PTY，页面从上次输出游标继续读取。受限的`flood`和`wait`命令只用于韧性验证。该原型不使用、也不计划引入桌面壳。
