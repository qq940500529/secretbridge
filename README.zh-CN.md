...
> [!IMPORTANT]
> **当前进入M2受控操作开发，M0安全门槛仍未全部关闭。** 本机Web管理端可以把密码和API令牌写入操作系统凭据库；获批的单次运行可以执行一个固定的PostgreSQL只读连接检查。秘密值没有读取或导出接口，任意SQL和带凭据Shell仍未开放。
...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的合成PTY。凭据元数据与PostgreSQL目标信息在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库，修改时要求已配对会话、精确Origin和预期版本，响应只显示“已配置／未配置”。PostgreSQL适配器强制验证证书与主机名，在可串行化只读事务中执行固定`SELECT 1`，只返回枚举状态；每条批准最多消费一次，配置漂移默认失效。普通终端不会获得凭据。状态接口明确标记当前只是“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[路线图](ROADMAP.md)。
