...
[![项目阶段：M3准备](https://img.shields.io/badge/项目阶段-M3%20安全验证-147D92)](ROADMAP.md)
...
> [!IMPORTANT]
> **当前进入M3安全试点准备，M0身份边界门槛仍未关闭。** 本机Web管理端可以保存系统凭据并完成受控审批/运行；MCP只能提出请求、查询、启动已批准的单次运行及取消。“安全验证”提供可重复自检证据，“平台证据”记录本机身份与IPC事实，“试点准入”汇总配置与外部门槛，“试点工作台”管理限时记录和固定验证矩阵。工作台不会自动连接远端，也不会把录入的引用冒充已经验证的业务授权。秘密值没有读取或导出工具／接口，任意SQL和带凭据Shell仍未开放。
...
当前已实现一次性配对、可过期／撤销的页面会话和可断线重连的合成PTY。凭据元数据与PostgreSQL目标信息在SQLite中带版本保存；密码和API令牌以不透明UUID写入操作系统凭据库。PostgreSQL适配器只执行固定只读检查并返回枚举状态，支持显式配置私有 CA 文件；每条批准最多消费一次，配置漂移默认失效。MCP只暴露8个固定工具，不包含审批决定、取密、SQL、Shell输入、连接串或目标地址。SQLite schema v11 保存可校验的安全验证、平台边界、试点准入、限时试点记录和逐项证据；报告不输出系统身份、路径、目标地址或秘密，也不声称完成授权或独立认证。状态接口仍明确标记“同用户兼容模式，身份隔离未验证”。其余能力及准入条件见[完整路线图](ROADMAP.md)。
...
| 运行并解读安全自检 | [安全验证与证据中心](docs/安全验证中心.md) |
| 采集并解读平台身份与 IPC 证据 | [平台身份与 IPC 证据](docs/平台身份与IPC证据.md) |
| 核对真实试点前的技术与外部门槛 | [安全试点准入中心](docs/试点准入中心.md) |
| 管理限时试点及固定验证矩阵 | [低权限试点工作台](docs/低权限试点工作台.md) |
...
项目处于M3安全试点准备阶段，M0身份边界验证仍在并行推进。欢迎参与安全评审、三平台适配、可访问性、测试与文档完善。开始贡献前请阅读[贡献指南](CONTRIBUTING.md)与[开发者入门](docs/开发者入门.md)。
