...
def check(root: Path) -> str:
    ...
    schema_source = (root / "backend/src/adapters/persistence/schema.rs").read_text(
        encoding="utf-8"
    )
    match = re.search(r"SCHEMA_VERSION: i64 = (\d+);", schema_source)
    if match is None:
        fail("SCHEMA_VERSION was not found")
    schema = int(match.group(1))
    package_tool = (root / "tools/release/package_release.py").read_text(encoding="utf-8")
    if f'"schema_version": {schema}' not in package_tool:
        fail("package_release.py schema metadata is out of date")
    current_documents = (
        root / "docs/README.md",
        root / "docs/development/架构设计.md",
        root / "docs/getting-started/安装与服务管理.md",
        root / "docs/user-guide/备份与恢复.md",
    )
