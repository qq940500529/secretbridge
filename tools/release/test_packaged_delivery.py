...
def accept(archive: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="sb-p-") as temporary:
        ...
        binary = package / "bin" / ("secretbridge.exe" if os.name == "nt" else "secretbridge")
        ...
        env = dict(
            ...
            SECRETBRIDGE_WEB_ROOT=str(package / "frontend"),
        ...
        try:
            ...
            (bad / "frontend" / "index.html").write_text("tampered", encoding="utf-8")
            ...
            malformed["files"][0]["path"] = "frontend/../../outside"
            ...
            build = broken / "frontend" / "secretbridge-build.json"
            ...
            index = upgraded / "frontend" / "index.html"
            index.write_text(
