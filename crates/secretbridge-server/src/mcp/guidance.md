...
3. For a changing local command, create a secure terminal and use `secretbridge_request_command` with an absolute executable and individual argv items. Put a longer non-secret script in bounded `stdin_content` when the program reads stdin; do not combine it with a stdin credential slot. A template is **not** a prerequisite. Save a template only if the user explicitly wants a stable operation reused; use `secretbridge_request_approval` for an existing enabled template. Ordinary non-secret terminal input uses `secretbridge_terminal_attach` and `secretbridge_terminal_write`; never write a secret or bypass approval with a shell.
