...
export function ActionTemplatesView({
  ...
  const text =
    language === "zh-CN"
      ? {
          ...
          safety:
            "PostgreSQL 连接检查仅执行内置 SELECT 1，只返回结构化状态；合成操作继续用于离线验证。",
          formCreate: "新建操作模板",
          formEdit: "编辑操作模板",
          name: "模板名称",
          ...
          operationLabels: {
            ...
            command_execution: "凭据任务（程序 / HTTP）",
          },
          scopeLabels: {
            status_only: "仅状态",
      ...
      : {
          ...
          safety:
            "The PostgreSQL connection check runs only a fixed SELECT 1 and returns structured status; synthetic operations remain available for offline validation.",
          ...
          descriptionPlaceholder:
            "Describe the use; do not enter commands, addresses or secrets",
          ...
          saveError:
            "The template could not be saved. Check the fields and retry.",
          deleteError:
            "The template could not be deleted. Referenced templates are preserved.",
          conflict:
            "The template changed elsewhere. The list has been refreshed.",
          deleteConfirm:
          ...
          operationLabels: {
            ...
            command_execution: "Credential task (program / HTTP)",
          },
          scopeLabels: {
  ...
  const [operation, setOperation] =
    useState<ApprovalOperation>("inspect_metadata");
  ...
  useEffect(() => {
    void listCredentialReferences(sessionToken)
    ...
    const base = {
      ...
      result_scope:
        operation === "command_execution"
          ? ("sanitized_output" as const)
          : scope,
      ...(operation === "command_execution" ? { command } : {}),
      ...(description.trim() ? { description } : {}),
      timeout_seconds: timeoutSeconds,
    };
    try {
      const saved = editing
        ? await updateActionTemplate(sessionToken, editing.id, {
            ...base,
            enabled,
    ...
    <section className="space-y-6">
          ...
          <div className="mt-2 grid gap-4 md:grid-cols-2">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label={text.scope}>
                <select
                  disabled={operation === "command_execution"}
                  value={
                    operation === "command_execution"
                      ? "sanitized_output"
                      : scope
                  ...
                  onChange={(event) =>
                    setScope(event.target.value as ApprovalResultScope)
                  }
                  className={inputClass}
                >
                  {Object.entries(text.scopeLabels).map(([value, label]) => (
                    <option key={value} value={value}>
                      {label}
                    </option>
                  ))}
          ...
          <div className="mt-5 flex justify-end gap-2">
            {editing && (
              <button
                type="button"
                onClick={reset}
                className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700"
              >
                <X className="size-4" />
                {text.cancel}
              </button>
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-cyan-700 px-5 py-2.5 text-sm font-semibold text-white hover:bg-cyan-800 disabled:opacity-50"
            >
              {editing ? (
      ...
      <div>
        ...
        {error && (
          <p
            role="alert"
            className="border-l-4 border-rose-400 bg-rose-50 px-4 py-3 text-sm text-rose-800"
          >
            {error}
          </p>
        )}
              ...
              <article key={item.id} className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <h3 className="mb-0 mt-2 text-sm font-semibold text-slate-950">
                      {item.name}
                    </h3>
                    <p className="mb-0 mt-1 text-sm text-slate-600">
                      {targetNames.get(item.target_id) ?? item.target_id} ·{" "}
                      {text.scopeLabels[item.result_scope]} ·{" "}
                      {item.timeout_seconds} {text.seconds} · {text.version}{" "}
                      {item.version}
                    </p>
                    {item.description && (
                  ...
                  <div className="flex shrink-0 gap-2">
                    ...
                    <button
                      ...
                      className={`${iconClass} hover:text-rose-700`}
                    >
...
const iconClass =
  "grid size-9 place-items-center rounded-lg border border-slate-200 text-slate-500 transition hover:border-cyan-300 hover:text-cyan-700";
