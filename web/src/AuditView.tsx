...
export function AuditView({ language, sessionToken }: { language: Language; sessionToken: string }) {
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex gap-3 border-l-4 border-indigo-400 bg-indigo-50 px-5 py-4 text-sm leading-6 text-indigo-950"><LockKeyhole className="mt-0.5 size-5 shrink-0" /><p className="m-0 font-medium">{text.policy}</p></div>
      ...
      {loading ? <p className={emptyClass}>{text.loading}</p> : visibleEvents.length === 0 ? <p className={emptyClass}>{text.empty}</p> : <ol className="enterprise-surface enterprise-table m-0 p-0">{visibleEvents.map((event) => { const run = runMap.get(event.run_id); return <li key={event.id} className="list-none p-5"><div className="flex items-start gap-4"><div className="grid size-9 shrink-0 place-items-center rounded-lg bg-slate-100 text-slate-600"><FileClock className="size-4" /></div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="m-0 text-sm font-semibold text-slate-950">{text.kinds[event.kind]}</h2><time className="text-xs text-slate-400">{new Intl.DateTimeFormat(language, { dateStyle: "medium", timeStyle: "medium" }).format(event.created_at_unix_ms)}</time></div><div className="mt-2 grid gap-2 text-xs text-slate-500 sm:grid-cols-2 lg:grid-cols-4"><span><strong>{text.sequence}：</strong>#{event.sequence}</span><span className="truncate"><strong>{text.run}：</strong>{event.run_id}</span><span className="truncate"><strong>{text.template}：</strong>{run ? (templateMap.get(run.action_template_id) ?? run.action_template_id) : "—"}</span><span className="truncate"><strong>{text.target}：</strong>{run ? (targetMap.get(run.target_id) ?? run.target_id) : "—"}</span></div></div></div></li>; })}</ol>}
    </section>
  );
}
