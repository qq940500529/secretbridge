    ...
    <section className="space-y-6">
      ...
      <details className="enterprise-disclosure enterprise-surface">
        <summary className="flex cursor-pointer items-center justify-between px-5 py-4 text-sm font-semibold text-slate-900 hover:bg-slate-50">
          {text.request}<span className="text-xs font-normal text-slate-500">＋</span>
        </summary>
        <form onSubmit={submit} className="border-t border-slate-200 px-5 py-5">
          <div className="flex flex-col gap-3 sm:flex-row">
            <label className="flex-1 text-sm font-semibold text-slate-700">
              ...
              <select required value={approvalId} onChange={(event) => setApprovalId(event.target.value)} className={inputClass}>
                <option value="">{text.choose}</option>
                {availableApprovals.map((approval) => <option key={approval.id} value={approval.id}>{approval.action_template_id ? templateNames.get(approval.action_template_id) : approval.operation} · {targetNames.get(approval.target_id)} · {text.approvalVersion} {approval.version}</option>)}
              </select>
            </label>
            <button type="submit" disabled={!approvalId || busy} className="mt-auto inline-flex h-[42px] items-center justify-center gap-2 rounded-lg bg-slate-950 px-5 text-sm font-semibold text-white hover:bg-cyan-800 disabled:opacity-50">
              <PlayCircle className="size-4" />{busy ? text.submitting : text.submit}
            </button>
          </div>
          {availableApprovals.length === 0 && !loading && <p className="mb-0 mt-3 text-sm text-amber-700">{text.none}</p>}
        </form>
      </details>

      <div>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="m-0 text-lg font-semibold text-slate-950">{text.runs}</h2>
          <button type="button" onClick={() => void refresh()} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-600"><RefreshCw className="size-3.5" />{runs.length}</button>
        </div>
        {error && <p role="alert" className="border-l-4 border-rose-400 bg-rose-50 p-3 text-sm text-rose-800">{error}</p>}
        {loading ? <p className={emptyClass}>{text.loading}</p> : runs.length === 0 ? <p className={emptyClass}>{text.empty}</p> : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${stateStyles[run.state]}`}><CircleDot className={`mr-1 inline size-3 ${run.state === "running" ? "animate-pulse" : ""}`} />{text.states[run.state]}</span>
                      <span className="text-xs font-medium text-slate-500">{templateNames.get(run.action_template_id) ?? run.operation}</span>
                    </div>
                    <h3 className="mb-0 mt-2 text-base font-semibold text-slate-950">{targetNames.get(run.target_id) ?? run.target_id}</h3>
                    <p className="mb-0 mt-2 text-xs text-slate-500"><Clock3 className="mr-1 inline size-3.5" />{new Intl.DateTimeFormat(language, { dateStyle: "medium", timeStyle: "medium" }).format(run.updated_at_unix_ms)} · {text.version} {run.version}</p>
                    {run.result_status && <p className="mb-0 mt-2 text-sm font-medium text-slate-600">{text.results[run.result_status]}</p>}
                  </div>
                  <div className="flex items-center gap-3">
                    <button type="button" onClick={() => void toggleEvents(run.id)} className="text-sm font-semibold text-cyan-700">{expandedRunId === run.id ? text.hideEvents : text.events}</button>
