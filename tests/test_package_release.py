...
class PackageReleaseTests(unittest.TestCase):
    ...
    def test_development_snapshot_records_dirty_state_and_embeds_source(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(package_release, "ROOT", root), patch.object(package_release, "command", side_effect=[" M Cargo.toml", "a" * 40, "Cargo.toml\0"]):
                result = package_release.source_snapshot(root / "source.tar.gz", True, 1_700_000_000)
    ...
    def test_sbom_contains_native_and_web_runtime_components(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cargo = {
                "packages": [
                    {"id": "app", "name": "secretbridge-server", "version": "0.2.0-beta.2", "source": None, "license": "AGPL-3.0-or-later", "manifest_path": "Cargo.toml"},
                    {"id": "serde", "name": "serde", "version": "1.0.229", "source": "registry", "license": "MIT OR Apache-2.0", "manifest_path": "serde/Cargo.toml"},
                ],
                "resolve": {"nodes": [
    ...
    def fixture(self, root):
        ...
        (web / "secretbridge-build.json").write_text(json.dumps({"format_version": 1, "version": "0.2.0-beta.2"}), encoding="utf-8")
    ...
    def test_manifest_covers_payload_source_notices_and_hashes(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def snapshot(target, _, epoch):
                ...
                return {"dirty": False, "corresponding_source": "SOURCE.tar.gz", "repository": "https://example.invalid/repository", "base_commit": "a" * 40, "source_date_epoch": epoch}
            sbom = {"bomFormat": "CycloneDX", "specVersion": "1.6", "version": 1}
            with patch.object(package_release, "ROOT", root), patch.object(package_release.subprocess, "check_output", return_value="0.2.0-beta.2"), patch.object(package_release, "release_epoch", return_value=1_700_000_000), patch.object(package_release, "source_snapshot", side_effect=snapshot), patch.object(package_release, "third_party_notices", return_value="license fixture"), patch.object(package_release, "software_bill_of_materials", return_value=sbom):
            ...
            self.assertTrue({"SOURCE.tar.gz", "SOURCE.json", "SBOM.cdx.json", "THIRD_PARTY_LICENSES.txt", "LICENSE", "web/index.html"} <= paths)
    ...
    def test_two_builds_with_the_same_inputs_are_byte_identical(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def snapshot(target, _, epoch):
                ...
                return {"dirty": False, "repository": "https://example.invalid/repository", "base_commit": "b" * 40, "source_date_epoch": epoch}
            sbom = {"bomFormat": "CycloneDX", "specVersion": "1.6", "version": 1}
            with patch.object(package_release, "ROOT", root), patch.object(package_release.subprocess, "check_output", return_value="0.2.0-beta.2"), patch.object(package_release, "release_epoch", return_value=1_700_000_000), patch.object(package_release, "source_snapshot", side_effect=snapshot), patch.object(package_release, "third_party_notices", return_value="license fixture"), patch.object(package_release, "software_bill_of_materials", return_value=sbom):
